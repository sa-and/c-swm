...
if __name__ == '__main__':
    ...
    # create one-hot id's
    if args.use_ids:
        id_tensor = torch.diag(torch.ones(input_shape[0]))[None, :].repeat(args.batch_size, 1, 1).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, data_batch in enumerate(train_loader):
            ...
            if args.use_ids:
                data_batch[0] = torch.cat([data_batch[0], id_tensor], dim=2)
                data_batch[-1] = torch.cat([data_batch[-1], id_tensor], dim=2)
            optimizer.zero_grad()
